...
app.post(
  ...
  async (req: Request, res: Response) => {
    const { rank, role, vessel, vesselType, contract, signOn, wage } =
      req.body ?? {}
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!EMAIL_RE.test(String(email))) {
      return res
        .status(400)
        .json({ error: "That doesn't look like a valid email." })
    }
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!label || !String(label).trim()) {
      return res
        .status(400)
        .json({ error: "A label is required (e.g. 'STCW Certificate')." })
    }
...
app.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (postedBy !== req.user!.id) {
      return res.status(403).json({
        error:
          'You can only view documents for applicants to positions you posted.',
      })
    }
